...
def main() -> None:
    parser = argparse.ArgumentParser(description="Preview or execute latest ensemble target-position plan after pre-trade exchange synchronization.")
    ...
    parser.add_argument("--allow-warning", action="store_true", help="Allow Demo/Testnet execution when pre-trade status is warning, never critical.")
    ...
    result = execute_synced_target_preview(
        cfg,
        preview,
    ...
    print(json.dumps({"preview": preview.to_dict(), "execution_result": result.to_dict()}, indent=2, ensure_ascii=False, default=str))
